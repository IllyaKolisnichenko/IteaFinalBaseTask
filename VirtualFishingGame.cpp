...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2) {
        fish.setFishFilePath("Fish.txt");
        water.setWaterFilePath("Water.txt");
    }
    else
    {
        std::string exeFilePath(argv[0]);
        std::string fishFileName(argv[1]);
        std::string waterFileName(argv[2]);

        char endSymbol;
        do {
            exeFilePath.pop_back();
            endSymbol = exeFilePath.back();
        } while (endSymbol != '\\');

        fish.setFishFilePath(exeFilePath + fishFileName);
        water.setWaterFilePath(exeFilePath + waterFileName);
    }
